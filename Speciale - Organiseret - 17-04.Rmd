...
data_no_rrp_no_na <- na.omit(subset(clean_data, rrp_i_p_lag1 == 0))
data_with_rrp_no_na <- na.omit(subset(clean_data, rrp_i_p_lag1 == 1))
...
clean_data_right_party <- subset(clean_data, left_party_lag1 == 0)
clean_data_left_party <- subset(clean_data, left_party_lag1 == 1)
...
large_parties <- subset(final_dataset, large_party_lag1 == 1)
small_parties <- subset(final_dataset, large_party_lag1 == 0)
...
# GDP growth variant
clean_data_high_gdp <- subset(final_dataset, high_gdp_growth_lag1 == 1 & !is.na(miljø_afhængig))
clean_data_low_gdp <- subset(final_dataset, high_gdp_growth_lag1 == 0 & !is.na(miljø_afhængig))
View(final_dataset)
# Composite economic index variant
clean_data_good_economy <- subset(final_dataset, good_economy_lag1 == 1 & !is.na(miljø_afhængig))
clean_data_bad_economy <- subset(final_dataset, good_economy_lag1 == 0 & !is.na(miljø_afhængig))
...
h3_high_gdp_density_test_alternativ <- DCdensity(clean_data_high_gdp$centered_lagged_pervote_samlet, c = 0, verbose = TRUE)
h3_low_gdp_density_test_alternativ <- DCdensity(clean_data_low_gdp$centered_lagged_pervote_samlet, c = 0, verbose = TRUE)

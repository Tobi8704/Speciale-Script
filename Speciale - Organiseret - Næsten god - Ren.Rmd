...
summary_stats <- final_dataset %>%
  dplyr::select(
    ...
    rrp_i_p_lag1, `GDP_per_capita_lag1`, Unemployment, `Inflation-consumer prices`, `good_economy_lag1`
  ) %>%
  summary()
...
country_summary <- final_dataset %>%
  ...
  summarise(
    ...
    avg_klimafokus = mean(miljø_afhængig, na.rm = TRUE)
  )
...
country_coverage_plot <- ggplot(country_summary, aes(x = reorder(countryname, n_observations), y = n_observations)) +
  ...
  coord_flip() +
  labs(
    title = "Antal Observation per Land",
    x = "Land",
...
time_coverage_plot <- ggplot(time_coverage, aes(x = Year, y = countryname, fill = has_obs)) +
  ...
  labs(
    title = "Tidslig dækning per Land",
    x = "Årstal",
    y = "Land",
    fill = "Har Data"
  ) +
...
h3_plot_high_gdp <- plot_rdd(
  ...
  x_label = "Green Party Vote Share (centered)",
  y_label = "Climate Position",
  title = "H3a: Climate Position med lav GDP"
)
...
h3_plot_low_gdp <- plot_rdd(
  ...
  x_label = "Green Party Vote Share (centered)",
  y_label = "Climate Position",
  title = "H3b: Climate Position med høj GDP"
)
...
h2_density_plot_with_rrp_alternativ <- ggplot(data_with_rrp, aes(x = centered_lagged_pervote_samlet)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, alpha = 0.5) +
  geom_density() +  # Use default bandwidth since we can't extract it easily
  geom_vline(xintercept = 0, linetype = "dashed", color = "red") +
  labs(
    title = "Density Test for Running Variable (With RRP)",
    subtitle = paste("McCrary test p-value:", round(h2_med_rrp_density_test_alternativ, 4)),
    x = "Green Party Vote Share (Centered)",
    y = "Density"
  ) +
  theme_minimal()

# For data without RRP
h2_density_plot_no_rrp_alternativ <- ggplot(data_no_rrp, aes(x = centered_lagged_pervote_samlet)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, alpha = 0.5) +
  geom_density() +  # Use default bandwidth
  geom_vline(xintercept = 0, linetype = "dashed", color = "red") +
  labs(
    title = "Density Test for Running Variable (No RRP)",
    subtitle = paste("McCrary test p-value:", round(h2_no_rrp_density_test_alternativ, 4)),
    x = "Green Party Vote Share (Centered)",
    y = "Density"
  ) +
  theme_minimal()

ggsave("Robusthedstests/H2 tests - Appendiks/h2_med_rrp_density_alternativ.png", plot = h2_density_plot_with_rrp_alternativ, width = 10, height = 6, dpi = 300)
ggsave("Robusthedstests/H2 tests - Appendiks/h2_no_rrp_density_alternativ.png", plot = h2_density_plot_no_rrp_alternativ, width = 10, height = 6, dpi = 300)

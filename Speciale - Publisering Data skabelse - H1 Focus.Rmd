...
# Snapshot pre-mainstream-filter data (threshold-merged, but before the
# avg_pervote_main >= 10 filter below removes anything). Kept so an alternative
# mainstream-party inclusion rule can be tried later (see the "H1 Focus:
# alternative mainstream-party definition" section at the end of this file)
# without re-running the threshold merge above.
mpds_european_1960_prefilter <- mpds_european_1960
...
```{r}
saveRDS(final_dataset, "final_dataset_publisering_H1_Focus.rds")
```

## 1.8 H1 Focus: alternative mainstream-party definition (diagnostic, not a replacement)

```{r alternative_mainstream_definition}
# Section 1.5 decides whether a party counts as "mainstream" (and is therefore
# eligible to be in the H1/H4/H5 sample at all) using that party's vote share
# AVERAGED ACROSS ITS ENTIRE OBSERVED HISTORY in the dataset -- potentially
# 1960s through 2020s, 60+ years. For a design whose observations span many
# decades per party, "was this party mainstream at the time of THIS election"
# is arguably the more correct question than "was it mainstream on average
# over its whole career" -- a party that declined (or grew) over time gets
# classified uniformly across its whole history either way, which can exclude
# exactly the elections closest to a Green party's threshold if that party's
# average is pulled below 10 by decades far from the election in question.
#
# This section re-derives final_dataset using a PER-ELECTION mainstream
# criterion (pervote >= 10 at that specific election) instead of the
# lifetime-average one, reusing every downstream step (merge with Green party
# data, parfam exclusion, RILE calculation, dedup, final variable creation)
# unchanged from above -- only the inclusion filter itself differs. This is a
# parallel, comparison dataset: it does NOT replace final_dataset above, and
# whether it helps or hurts H1's near-cutoff sample size is an open empirical
# question answered by the diagnostic at the end of this chunk, not assumed.
mpds_european_1960_alt <- mpds_european_1960_prefilter %>%
  group_by(party) %>%
  mutate(avg_pervote_main = mean(pervote, na.rm = TRUE)) %>%
  ungroup() %>%
  filter(pervote >= 10) %>%
...
full_dataset_alt <- merge(
  mpds_european_1960_alt,
  green_party_data_sub,
  by = c("countryname", "edate"),
  all.x = FALSE
)

full_dataset_alt <- full_dataset_alt %>%
  filter(parfam != "10")

full_dataset_alt <- calculate_rile_lowe(full_dataset_alt)

if (!"per416_2" %in% names(full_dataset_alt) && "per416_2.x" %in% names(full_dataset_alt)) {
  full_dataset_alt <- full_dataset_alt %>% dplyr::rename(per416_2 = per416_2.x)
}

final_dataset_alt <- full_dataset_alt %>%
  dplyr::select(
    countryname, edate, country, party, oecdmember, eumember, date,
    partyname, partyabbrev, parfam, absseat, per410, per416, per501, per416_2,
    per607, per608,
...
# Same derived-variable construction as section 1.7, applied to the
# alternative-inclusion dataset.
final_dataset_alt <- final_dataset_alt %>%
  mutate(
    pervote_samlet = as.numeric(pervote_samlet),
    lagged_pervote_samlet = as.numeric(lagged_pervote_samlet),
    centered_pervote_samlet = pervote_samlet - threshold,
...
final_dataset_alt <- final_dataset_alt %>% dplyr::arrange(party, edate)

saveRDS(final_dataset_alt, "final_dataset_publisering_H1_Focus_altmainstream.rds")

# --- Diagnostic: does the alternative definition actually add near-cutoff power? ---
# H1's non-parametric MSE-optimal bandwidth in the CK-style table was ~1.1
# (centered_lagged_pervote_samlet). Comparing row/cluster counts inside that
# window, old vs. new mainstream definition, answers the actual question --
# adding mainstream parties far from the cutoff doesn't help the non-parametric
# H1 estimate at all, only additions inside this window do.
h1_window <- 1.1

compare_row <- function(data, label) {
  d <- data[complete.cases(data[, c("miljø_afhængig", "centered_lagged_pervote_samlet",
                                     "lagged_i_parlament", "country_election_id")]), ]
  in_window <- d[abs(d$centered_lagged_pervote_samlet) <= h1_window, ]
  data.frame(
    Dataset = label,
    Total_rows = nrow(d),
    Rows_in_H1_window = nrow(in_window),
    Clusters_in_H1_window = length(unique(in_window$country_election_id))
  )
}

mainstream_definition_comparison <- rbind(
  compare_row(final_dataset, "Original (lifetime-average >= 10%)"),
  compare_row(final_dataset_alt, "Alternative (per-election >= 10%)")
)
print(mainstream_definition_comparison)
write.csv(mainstream_definition_comparison,
          "mainstream_definition_comparison_H1_Focus.csv", row.names = FALSE)
```

...
pacman::p_load(
  ...
  # Visualization
  ggplot2, patchwork, scales, RColorBrewer, gridExtra, ggridges, corrplot, ggh4x, modelsummary,
...
# Save the plot
ggsave("figures/h1_rdd_plot.png", h1_plot_rdd, width = 10, height = 6, dpi = 300)
...
ggsave("figures/h1_rdd_plot_zoom.png", h1_plot_rdd_zoom, width = 10, height = 6, dpi = 300)
...
writeLines(main_results_table, file.path(getwd(), "h1_main_results.md"))
...
# Save the table to a file
writeLines(robustness_table, "h1_robustness_table.md")
...
h1_stability_plot <- create_coefficient_stability_plot(
  ...
  polynomials = c(1, 2),
  covariates = c("country", "edate"),
  title = "H1: Coefficient Stability Across Bandwidths and Polynomial Orders"
)

# Save the plot
ggsave("figures/h1_stability_plot.png", h1_stability_plot$plot, width = 10, height = 6, dpi = 300)

# 3.2 Placebo Tests ----------

# Run placebo threshold test
...
# Save the plot
ggsave("figures/h1_placebo_test.png", h1_placebo_plot, width = 10, height = 6, dpi = 300)

# 4. DENSITY TEST (MANIPULATION TEST) ----------------

# Check for potential manipulation of the running variable
density_test <- rddensity(final_dataset$centered_lagged_pervote_samlet, c = 0)
density_test_summary <- summary(density_test)

# Plot density test
density_plot_h1 <- rdplotdensity(density_test, 
                             final_dataset$centered_lagged_pervote_samlet,
                             title = "Density Test of Running Variable")

# Hent selve plottet fra listen og gem det med ggsave
density_plot_object_h1 <- rdplotdensity(density_test, 
                                    clean_data$centered_lagged_pervote_samlet,
                                    title = "Density Test of Running Variable")

# Gem plottet - tilgå det første element i listen som ofte er selve plottet
ggsave("figures/h1_density_test_h1.png", plot = density_plot_object$Estplot, width = 10, height = 6, dpi = 300)

# 5. COVARIATE BALANCE TEST ----------------

# Select a few important covariates to check for balance
covariates_to_check <- c("rile_lowe", "GDP_per_capita", "Unemployment")

# Create empty dataframe to store results
balance_results <- data.frame(
  Covariate = character(0),
  RDD_Estimate = numeric(0),
  Std_Error = numeric(0),
  P_value = numeric(0)
)

# Run balance checks for each covariate
for (cov in covariates_to_check) {
  if (cov %in% names(final_dataset)) {
    # Run RDD on the covariate
...
# Create a table of balance test results
balance_table <- knitr::kable(balance_results, 
                             caption = "Covariate Balance Test Results",
                             col.names = c("Covariate", "RDD Estimate", "Std. Error", "p-value"))

# Save the table
writeLines(balance_table, "tables/h1_balance_test.md")

# 6. SAVE SIMPLIFIED RESULTS OBJECT ----------------

# Create a list with main results for easier reference
h1_results <- list(
  main_estimate = summary_h1$Estimate[1],
  std_error = summary_h1$SE[1],
  p_value = summary_h1$pv[1],
  confidence_interval = summary_h1$ci,
  bandwidth = summary_h1$bws[1, 1],
  effective_n = c(left = summary_h1$N_h[1], right = summary_h1$N_h[2]),
  placebo_p_value = h1_placebo_threshold$empirical_p_value,
  density_p_value = density_test_summary$p_value
)

# Save results object for later use
saveRDS(h1_results, "results/h1_results.rds")
  
```

## 3.2 H2: Radical right parties in parliament reduce climate focus in mainstream parties
```{r H2}
## Simple linear model with both main effects
h2_model_simple <- lm(miljø_afhængig ~ centered_lagged_pervote_samlet + lagged_i_parlament + rrp_i_p_lag1,
                      data = clean_data)

# Interaction model between green party presence and RRP presence
h2_model_interaction <- lm(miljø_afhængig ~ lagged_i_parlament * rrp_i_p_lag1 + centered_lagged_pervote_samlet, 
                          data = clean_data)
...
models_list_h2_simpel <- list(h2_model_simple, h2_model_interaction, h2_model_3way)
  
# Create stargazer output with p-values and custom significance levels
stargazer(models_list_h2_simpel,
          type = "text",
          out = "models_list_h2_simpel.html",
          star.cutoffs = c(0.1, 0.05, 0.01),  # Significance thresholds
          star.char = c("*", "**", "***"),  # Corresponding symbols
          column.labels = c("LM", "LM Interaktion", "LM Trevejsmodel"),
          title = "Comparison of Different Modeling Approaches for H2",
          report=('vc*p'),
          covariate.labels = c(
          "Grønne stemmer (centeret)", 
          "Tidligere parlamentstilstand", 
          "RRP i parlament (lag1)",
          "Grønne × Parlament", 
          "Grønne × RRP", 
          "Parlament × RRP", 
          "Trevejsinteraktion"
          ),
          dep.var.labels = "miljø_afhængig"
)
...
# Add country and election fixed effects
h2_model_fe <- lm(miljø_afhængig ~ lagged_i_parlament * rrp_i_p_lag1 + 
                 centered_lagged_pervote_samlet + factor(country) + factor(edate), 
                 data = clean_data)
...
# Models on split samples
h2_model_no_rrp <- lm(miljø_afhængig ~ centered_lagged_pervote_samlet * lagged_i_parlament, 
                      data = data_no_rrp)

h2_model_with_rrp <- lm(miljø_afhængig ~ centered_lagged_pervote_samlet * lagged_i_parlament, 
                        data = data_with_rrp)

# Summarize results
summary(h2_model_no_rrp)
summary(h2_model_with_rrp)

# Prepare models for comparison
models_list_h2_simpel_med_og_uden <- list(h2_model_no_rrp, h2_model_with_rrp)
  
# Create stargazer output with p-values and custom significance levels
stargazer(models_list_h2_simpel_med_og_uden,
          type = "text",
          out = "models_list_h2_simpel_med_og_uden.html",
          star.cutoffs = c(0.1, 0.05, 0.01),  # Significance thresholds
          star.char = c("*", "**", "***"),  # Corresponding symbols
          column.labels = c("LM uden HRP", "LM med HRP"),
          title = "Comparison of Different Modeling Approaches for H2",
          report=('vc*p'),
          covariate.labels = c(
          "Grønne stemmer (centeret)", 
          "Tidligere parlamentstilstand", 
          "Interaktion: Grønne x Parlament"
          ),
          dep.var.labels = "miljø_afhængig"
)
...
grid.arrange(h2_plot_no_rrp, h2_plot_with_rrp, 
             ncol = 2,
             widths = c(1, 1),
             top = "Comparison of Green Party Effect with/without RRP in Parliament")

## Split data for no RRP/with RRP and zero/non-zero values
...
# TWO-PART MODEL FOR NO RRP SUBSAMPLE
...
# TWO-PART MODEL FOR WITH RRP SUBSAMPLE
# Part 1: Logit model for predicting non-zero climate focus
...
# Prepare models for comparison
models_list_h2_two_part <- list(h2_binary_no_rrp, h2_continuous_no_rrp, h2_binary_with_rrp, h2_continuous_with_rrp)
  
# GLM-modeller
stargazer(h2_binary_no_rrp, h2_binary_with_rrp,
          type = "text",
          out = "models_list_h2_two_part_binary.html",
          title = "GLM Models for H2 (Binær afhængig variabel)",
          star.cutoffs = c(0.1, 0.05, 0.01),  # Significance thresholds
          star.char = c("*", "**", "***"),  # Corresponding symbols
          report=('vc*p'),
          covariate.labels = c(
          ...
          dep.var.labels = "miljø_afhængig",
          column.labels = c("No RRP", "With RRP"))

# LM-modeller
stargazer(h2_continuous_no_rrp, h2_continuous_with_rrp,
          type = "text",
          out = "models_list_h2_two_part_continuous.html",
          title = "LM Models for H2 (Kontinuert afhængig variabel)",
          star.cutoffs = c(0.1, 0.05, 0.01),  # Significance thresholds
          star.char = c("*", "**", "***"),  # Corresponding symbols
          report=('vc*p'),
          covariate.labels = c(
...
pred_range <- seq(-5, 5, by = 0.1)

# NO RRP PREDICTIONS
# Binary predictions
pred_binary_no_rrp_0 <- predict(h2_binary_no_rrp, 
                              newdata = data.frame(centered_lagged_pervote_samlet = pred_range,
                                                  lagged_i_parlament = 0),
                              type = "response")
pred_binary_no_rrp_1 <- predict(h2_binary_no_rrp, 
                              newdata = data.frame(centered_lagged_pervote_samlet = pred_range,
                                                  lagged_i_parlament = 1),
                              type = "response")

# Continuous predictions
pred_cont_no_rrp_0 <- predict(h2_continuous_no_rrp, 
                            newdata = data.frame(centered_lagged_pervote_samlet = pred_range,
                                                lagged_i_parlament = 0))
pred_cont_no_rrp_1 <- predict(h2_continuous_no_rrp, 
                            newdata = data.frame(centered_lagged_pervote_samlet = pred_range,
                                                lagged_i_parlament = 1))

# WITH RRP PREDICTIONS
# Binary predictions
pred_binary_with_rrp_0 <- predict(h2_binary_with_rrp, 
                                newdata = data.frame(centered_lagged_pervote_samlet = pred_range,
                                                    lagged_i_parlament = 0),
                                type = "response")
pred_binary_with_rrp_1 <- predict(h2_binary_with_rrp, 
                                newdata = data.frame(centered_lagged_pervote_samlet = pred_range,
                                                    lagged_i_parlament = 1),
                                type = "response")

# Continuous predictions
pred_cont_with_rrp_0 <- predict(h2_continuous_with_rrp, 
                              newdata = data.frame(centered_lagged_pervote_samlet = pred_range,
                                                  lagged_i_parlament = 0))
pred_cont_with_rrp_1 <- predict(h2_continuous_with_rrp, 
                              newdata = data.frame(centered_lagged_pervote_samlet = pred_range,
                                                  lagged_i_parlament = 1))

# Create data frames for magnitude plotting
plot_data_magnitude <- data.frame(
  centered_lagged_pervote_samlet = rep(pred_range, 4),
  magnitude = c(pred_cont_no_rrp_0, pred_cont_no_rrp_1, 
                pred_cont_with_rrp_0, pred_cont_with_rrp_1),
  green_status = rep(rep(c("No Green in Parl.", "Green in Parl."), 
                      each=length(pred_range)), 2),
  rrp_status = rep(c("Without RRP", "With RRP"), each=length(pred_range)*2)
)

# Create ggplot for magnitude
p2 <- ggplot(plot_data_magnitude, aes(x=centered_lagged_pervote_samlet, 
                                     y=magnitude, 
                                     color=green_status)) +
  geom_line(size=1.2) +
  geom_vline(xintercept=0, linetype="dashed") +
  facet_wrap(~ rrp_status) +
  labs(title="Magnitude of Climate Position",
       x="Green Party Vote Share",
       y="Climate Position Value") +
...
# Save combined plot
ggsave("h2_two_part_model_complete.png", plot=combined_plot, width=12, height=16, dpi=300)

## Quantile regression for No RRP
h2_quantile_no_rrp <- rq(miljø_afhængig ~ centered_lagged_pervote_samlet * lagged_i_parlament, 
                       tau = 0.5, data = data_no_rrp)

# Quantile regression for With RRP
h2_quantile_with_rrp <- rq(miljø_afhængig ~ centered_lagged_pervote_samlet * lagged_i_parlament, 
                         tau = 0.5, data = data_with_rrp)

# Multiple quantiles for more comprehensive analysis
h2_multi_quantile_no_rrp <- rq(miljø_afhængig ~ centered_lagged_pervote_samlet * lagged_i_parlament, 
                             tau = c(0.25, 0.5, 0.75), data = data_no_rrp)

h2_multi_quantile_with_rrp <- rq(miljø_afhængig ~ centered_lagged_pervote_samlet * lagged_i_parlament, 
                               tau = c(0.25, 0.5, 0.75), data = data_with_rrp)

# Summarize results
summary(h2_quantile_no_rrp)
summary(h2_quantile_with_rrp)
summary(h2_multi_quantile_no_rrp)
summary(h2_multi_quantile_with_rrp)

# Prepare models for comparison
models_list_h2_kvartil_regression <- list(h2_quantile_no_rrp, h2_quantile_with_rrp, h2_multi_quantile_no_rrp, h2_multi_quantile_with_rrp)

modelsummary(models_list_h2_kvartil_regression,
             output = "models_list_h2_kvartil_regression.html",
             stars = c("*" = .1, "**" = .05, "***" = .01),
             coef_rename = c(
               "centered_lagged_pervote_samlet" = "Grønne stemmer (centeret)",
               "lagged_i_parlament" = "Tidligere parlamentstilstand",
               "centered_lagged_pervote_samlet:lagged_i_parlament" = "Interaktion: Grønne x Parlament"
             ),
             gof_omit = "AIC|BIC|Log.Lik",
             title = "Kvantilregressionsmodeller for H2")

# MM-type robust regression for No RRP
h2_robust_reg_no_rrp <- rlm(miljø_afhængig ~ centered_lagged_pervote_samlet * lagged_i_parlament, 
                          data = data_no_rrp, method = "MM")

# MM-type robust regression for With RRP
...
modelsummary(
  list("Robust uden RRP" = h2_robust_reg_no_rrp,
       "Robust med RRP" = h2_robust_reg_with_rrp),
  stars = c("*" = .1, "**" = .05, "***" = .01),
  coef_rename = c(
    "centered_lagged_pervote_samlet" = "Grønne stemmer (centeret)",
    "lagged_i_parlament" = "Tidligere parlamentstilstand",
  ...
  output = "h2_robust_models_table.html",
  title = "Robuste MM-regressioner for H2"
)
...
results_df_no_rrp <- run_rdd_robustness(
  ...
  running_var = "centered_lagged_pervote_samlet",
  bw_list = c(1, 1.5, 2, 2.5, 3),
  polynomials = c(1, 2),  # Focus on linear and quadratic models
...
write.csv(combined_results, "h2_rdd_robust.csv", row.names = FALSE)

# Compare coefficients across groups using stability plot
h2_comparison_plot <- compare_coefficient_stability(
  ...
  running_var = "centered_lagged_pervote_samlet",
  bw_list = seq(1, 3, by = 0.5),
  polynomial = 1,  # Linear model for comparison
  covariates = c("country", "edate"),
  title = "H2: Effect of Green Parties Moderated by Radical Right Parties"
)
...
## Make predictions based on the three-way interaction model
...
pred_data$rrp_status <- factor(pred_data$rrp_i_p_lag1, 
                            levels = c(0, 1),
                            labels = c("No RRP in Parliament", "RRP in Parliament"))

pred_data$gp_status <- factor(pred_data$lagged_i_parlament,
                           levels = c(0, 1),
                           labels = c("Green Party without Seats", "Green Party with Seats"))

# Plot the interaction
interaction_plot <- ggplot(pred_data, aes(x = centered_lagged_pervote_samlet, 
                                        y = predicted, 
                                        color = gp_status, 
                                        linetype = gp_status)) +
  geom_line() +
  ...
  theme_minimal() +
  labs(
    title = "H2: Predicted Climate Position Based on Three-way Interaction Model",
...
summary_table <- data.frame(
  Method = c(
    "OLS", 
    "Two-part (Probability)", 
    "Two-part (Magnitude)", 
    "Robust Regression", 
    "RDD (p=1, bw=2)"
  ),
  Without_RRP = c(
    coef(h2_model_no_rrp)["lagged_i_parlament"],
    ...
    coef(h2_rdd_no_rrp)[1]
  ),
  Without_RRP_SE = c(
    summary(h2_model_no_rrp)$coefficients["lagged_i_parlament", "Std. Error"],
    ...
    summary(h2_robust_reg_no_rrp)$coefficients["lagged_i_parlament", "Std. Error"],
    as.numeric(gsub(",", ".", results_df_no_rrp$StdErr[results_df_no_rrp$Polynomial == 1 & results_df_no_rrp$Bandwidth == 2])),
    summary(h2_rdd_no_rrp)$se[1]
  ),
  With_RRP = c(
    coef(h2_model_with_rrp)["lagged_i_parlament"],
  ...
  With_RRP_SE = c(
    summary(h2_model_with_rrp)$coefficients["lagged_i_parlament", "Std. Error"],
    ...
    summary(h2_robust_reg_with_rrp)$coefficients["lagged_i_parlament", "Std. Error"],
    as.numeric(gsub(",", ".", results_df_with_rrp$StdErr[results_df_with_rrp$Polynomial == 1 & results_df_with_rrp$Bandwidth == 2]))
  )
)

# Beregn forskel mellem effekter
summary_table$Difference <- summary_table$With_RRP - summary_table$Without_RRP

# Tilføj signifikansstjerner baseret på p-værdier
add_stars <- function(estimates, se, alpha_levels = c(0.1, 0.05, 0.01)) {
  # Beregn p-værdier
  p_values <- 2 * (1 - pnorm(abs(estimates / se)))
  
  # Tilføj stjerner
  stars <- rep("", length(p_values))
  stars[p_values < alpha_levels[1]] <- "*"
  stars[p_values < alpha_levels[2]] <- "**"
  stars[p_values < alpha_levels[3]] <- "***"
  
  # Kombinér estimater og stjerner
  return(paste0(sprintf("%.3f", estimates), stars))
}

# Formater estimater med stjerner
summary_table$Without_RRP_fmt <- add_stars(summary_table$Without_RRP, summary_table$Without_RRP_SE)
summary_table$With_RRP_fmt <- add_stars(summary_table$With_RRP, summary_table$With_RRP_SE)

# Beregn p-værdier
summary_table$Without_RRP_p <- 2 * (1 - pnorm(abs(summary_table$Without_RRP / summary_table$Without_RRP_SE)))
summary_table$With_RRP_p    <- 2 * (1 - pnorm(abs(summary_table$With_RRP / summary_table$With_RRP_SE)))

# Formater p-værdier
summary_table$Without_RRP_p_fmt <- ifelse(
  is.na(summary_table$Without_RRP_p), "NA",
  sprintf("%.3f", summary_table$Without_RRP_p)
)

summary_table$With_RRP_p_fmt <- ifelse(
  is.na(summary_table$With_RRP_p), "NA",
  sprintf("%.3f", summary_table$With_RRP_p)
)

# Lav den endelige tabel inkl. p-værdier
final_table <- summary_table[, c("Method", 
                                 "Without_RRP_fmt", "Without_RRP_p_fmt", 
                                 "With_RRP_fmt", "With_RRP_p_fmt", 
                                 "Difference")]

colnames(final_table) <- c("Method", 
                           "Without RRP (est.)", "Without RRP (p)", 
                           "With RRP (est.)", "With RRP (p)", 
                           "Difference")
...
knitr::kable(final_table, 
            caption = "Summary of Key Results Across Different Methods",
            align = c("l", "r", "r", "r"))
```

...
pacman::p_load(
  ...
  # Visualization
  ggplot2, patchwork, scales, RColorBrewer, gridExtra, ggridges, corrplot, ggh4x, modelsummary, kableExtra, UpSetR,
...
# Find kolonner med manglende værdier
...
# Find kolonner der har manglende værdier
columns_with_missing <- missing_data_overview$variable

# Opret et subset af data med kun disse kolonner
missing_data_subset <- final_dataset %>%
  dplyr::select(all_of(columns_with_missing))

# Lav UpSet plot
tryCatch({
  # Opret større plot med justerede navne
  missing_patterns_plot <- gg_miss_upset(missing_data_subset, 
                                        nsets = length(columns_with_missing)) +
    # Større plotstørrelse
    theme(
      plot.margin = margin(1, 1, 1, 5, "cm"),  # Mere plads i venstre side
      axis.text.y = element_text(size = 10),    # Større tekst på y-aksen
      axis.title = element_text(size = 12)      # Større aksetitler
    )
  
  # Vis plot med specificeret størrelse
  ggsave("missing_data_plot.png", missing_patterns_plot, 
         width = 12, height = 8, dpi = 300)
  print(missing_patterns_plot)
  
}, error = function(e) {
  cat("Fejl med gg_miss_upset:", e$message, "\n")
  
  # Alternativ visualisering hvis UpSet ikke virker
  vis_miss_plot <- vis_miss(missing_data_subset, 
                          cluster = TRUE) +  # Klynger lignende mønstre
          theme(
            axis.text.x = element_text(angle = 45, hjust = 1, size = 10),
            plot.margin = margin(1, 1, 1, 1, "cm")
          )
  
  ggsave("missing_data_vis.png", vis_miss_plot, 
         width = 12, height = 8, dpi = 300)
  print(vis_miss_plot)
})
...
# Save the table to a file
writeLines(robustness_table, "Analyse/H1/h1_robustness_table.md")
...
# Summarize results
sink("Analyse/H2/both_summaries.txt")
cat("Summary of h2_rdd_no_rrp:\n\n")
print(summary(h2_rdd_no_rrp))
cat("\n\n----------------------------------------\n\n")
cat("Summary of h2_rdd_with_rrp:\n\n")
print(summary(h2_rdd_with_rrp))
sink()

print(h2_rdd_no_rrp)
print(h2_rdd_with_rrp)
...
# Først gemmer vi det kombinerede plot i en variabel
combined_gdp_plot <- grid.arrange(h3_plot_high_gdp, h3_plot_low_gdp, 
             ncol = 2,
             widths = c(1, 1),
             top = "Comparison of Green Party Effect with høj/lav GDP")

# Derefter bruger vi ggsave til at gemme plottet
ggsave("Analyse/H3/comparison_green_party_gdp_effect.png", combined_gdp_plot, width = 12, height = 6)
...
sink("Analyse/H3/both_gdp_summaries.txt")
cat("Summary of h3_rdd_high_gdp:\n\n")
print(summary(h3_rdd_high_gdp))
cat("\n\n----------------------------------------\n\n")
cat("Summary of h3_rdd_low_gdp:\n\n")
print(summary(h3_rdd_low_gdp))
sink()
...
if (requireNamespace("knitr", quietly = TRUE)) {
  ...
  write(md_table, file = "Analyse/H3/h3_robustness_table.md")
...
# Gem separate summaries til tekstfiler
capture.output(summary(h3_rdd_high_gdp), file = "Analyse/H3/summary_high_gdp.txt")
capture.output(summary(h3_rdd_low_gdp), file = "Analyse/H3/summary_low_gdp.txt")

# Eller gem begge summaries i samme fil
sink("Analyse/H3/both_gdp_summaries.txt")
cat("Summary of h3_rdd_high_gdp:\n\n")
print(summary(h3_rdd_high_gdp))
cat("\n\n----------------------------------------\n\n")
...
# Gem output for højreorienterede partier
sink("Analyse/H4/h4_rdd_right_summary.txt")
print(summary(h4_rdd_right))
sink()

# Gem output for venstreorienterede partier
sink("Analyse/H4/h4_rdd_left_summary.txt")
print(summary(h4_rdd_left))
sink()

# Hvis du vil gemme hele objekterne til fremtidig brug
save(h4_rdd_right, file = "Analyse/H4/h4_rdd_right.RData")
save(h4_rdd_left, file = "Analyse/H4/h4_rdd_left.RData")
...
write.csv(results_df_h4, "Analyse/H4/results_df_h4_full_sample.csv", row.names = FALSE)
...
# Gem RData-objekter for fuld genbrug
save(results_df_h4, file = "Analyse/H4/results_df_h4_full_sample.RData")
save(results_df_h4_right, file = "Analyse/H4/results_df_h4_right_parties.RData")
save(results_df_h4_left, file = "Analyse/H4/results_df_h4_left_parties.RData")
save(combined_results_ideology, file = "Analyse/H4/combined_results_ideology.RData")

# Gem tekstrapporter
sink("Analyse/H4/results_summary_full_sample.txt")
print(results_df_h4)
sink()

sink("Analyse/H4/results_summary_right_parties.txt")
print(results_df_h4_right)
sink()

sink("Analyse/H4/results_summary_left_parties.txt")
print(results_df_h4_left)
sink()

# Gem tabeller som PDF ved hjælp af kun knitr
if (requireNamespace("knitr", quietly = TRUE)) {
  # Fuld stikprøve tabel
  pdf("Analyse/H4/robustness_table_full_sample.pdf")
  print(knitr::kable(results_df_h4, 
                     format = "pipe", 
                     caption = "H4: Robusthedstest med forskellige båndbredder og polynomiegrader"))
  dev.off()
  
  # Kombineret ideologi tabel
  pdf("Analyse/H4/robustness_table_by_ideology.pdf")
...
# Gem output for store partier
sink("Analyse/H5/h5_large_rdd_summary.txt")
print(summary(h5_large_rdd))
sink()

# Gem output for små partier
sink("Analyse/H5/h5_small_rdd_summary.txt")
print(summary(h5_small_rdd))
sink()

# Gem hele RD-robuste objekter til fremtidig brug
save(h5_large_rdd, file = "Analyse/H5/h5_large_rdd.RData")
save(h5_small_rdd, file = "Analyse/H5/h5_small_rdd.RData")
...
# Gem knitr-tabel som tekst, hvis knitr er tilgængeligt
if (requireNamespace("knitr", quietly = TRUE)) {
 # Gem tabellen som en tekstfil med den ønskede formatering
 knitr::kable(combined_results_size, 
              format = "pipe", 
   ...
   cat(file = "Analyse/H5/robustness_table_size.txt")
}
```
...
h1_density_test <- rddensity(h1_analysis_data$centered_lagged_pervote_samlet, c = 0)
...
h2_no_rrp_density_test <- rddensity(data_no_rrp$centered_lagged_pervote_samlet, c = 0)
...
h2_med_rrp_density_test <- rddensity(data_with_rrp$centered_lagged_pervote_samlet, c = 0)
...
h2_med_rrp_density_plot <- rdplotdensity(h2_med_rrp_density_test, 
                                           ...
                                           title = "H3: Density Test (Uden Højreradikale Partier)")

summary(h2_no_rrp_density_test)
summary(h2_med_rrp_density_test)

h2_no_rrp_density_test_alternativ <- DCdensity(data_no_rrp$centered_lagged_pervote_samlet, c = 0, verbose = TRUE)
h2_med_rrp_density_test_alternativ <- DCdensity(data_with_rrp$centered_lagged_pervote_samlet, c = 0, verbose = TRUE)

print(h2_no_rrp_density_test_alternativ)
print(h2_med_rrp_density_test_alternativ)

# Save density plots
ggsave("h2_no_rrp_density.png", plot = h2_no_rrp_density_plot$Estplot, width = 10, height = 6, dpi = 300)
ggsave("h2_med_rrp_density.png", plot = h2_med_rrp_density_plot$Estplot, width = 10, height = 6, dpi = 300)
...
h3_high_gdp_density_test <- rddensity(h3_high_gdp$centered_lagged_pervote_samlet, c = 0)
...
h3_low_gdp_density_test <- rddensity(h3_low_gdp$centered_lagged_pervote_samlet, c = 0)
...
h4_left_density_test <- rddensity(clean_data_left_party$centered_lagged_pervote_samlet, c = 0)
...
h4_right_density_test <- rddensity(clean_data_right_party$centered_lagged_pervote_samlet, c = 0)
...
write.csv(h5_test_summary, "h5_test_summary.csv", row.names = FALSE)

...
pacman::p_load(
  ...
  # Visualization
  ggplot2, patchwork, scales, RColorBrewer, gridExtra, ggridges, corrplot,
...
party_family_climate <- final_dataset %>%
  dplyr::mutate(
    party_family = case_when(
      parfam == "10" ~ "Green",
      parfam == "20" ~ "Socialist",
      parfam == "30" ~ "Social Democratic/Other left parties",
      parfam == "40" ~ "Liberal",
      ...
      parfom == "98" ~ "Electoral alliances w/o leader"
      TRUE ~ "Other"
    )
  ) %>%
...
# This needs to be run with the actual data
tryCatch({
  ...
  density_data <- data.frame(
    x = seq(min(final_dataset$centered_lagged_pervote_samlet, na.rm = TRUE),
            max(final_dataset$centered_lagged_pervote_samlet, na.rm = TRUE),
            length.out = 1000)
  )
  
  # Get estimated densities
  density_fit <- rddensity::rddensity_plot(density_test, density_data$x)
  
  density_data$left_density <- density_fit$hat_f_l
  density_data$right_density <- density_fit$hat_f_r
  
  # For visualization
  density_test_plot <- ggplot(density_data, aes(x = x)) +
    geom_line(aes(y = left_density), color = "blue", size = 1) +
    geom_line(aes(y = right_density), color = "red", size = 1) +
    geom_vline(xintercept = 0, linetype = "dashed") +
    labs(
      title = "Density Test Around Threshold",
      subtitle = paste("p-value:", round(density_test_summary$test$p_jk, 4)),
      x = "Centered Vote Share",
      y = "Density"
    ) +
    theme_minimal()
}, error = function(e) {
  # If rddensity fails, create a basic histogram
  density_test_plot <- ggplot(final_dataset, aes(x = centered_lagged_pervote_samlet)) +
    geom_histogram(binwidth = 0.25, fill = "steelblue", color = "darkblue", alpha = 0.7) +
    geom_vline(xintercept = 0, linetype = "dashed", color = "red") +
    labs(
      title = "Distribution Around Threshold (rddensity failed)",
      x = "Centered Vote Share",
      y = "Count"
    ) +
    theme_minimal()
...
econ_time_plot <- final_dataset %>%
  dplyr::group_by(Year) %>%
  summarise(
    gdp_growth = mean(`GDP per capita growth`, na.rm = TRUE),
    unemployment = mean(Unemployment, na.rm = TRUE),
    inflation = mean(`Inflation-consumer prices`, na.rm = TRUE)
  ) %>%
  pivot_longer(cols = c(gdp_growth, unemployment, inflation),
               names_to = "indicator",
               values_to = "value") %>%
...
conditional_means_plot <- final_dataset %>%
  dplyr::mutate(bin = cut(centered_lagged_pervote_samlet, 
                  breaks = seq(-10, 10, by = 1), 
                  include.lowest = TRUE)) %>%
  group_by(bin, factor_lagged_i_parlament) %>%
  summarise(
  ...
  ggplot(aes(x = median_bin, y = mean_miljø, color = factor_lagged_i_parlament)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = mean_miljø - se_miljø, ymax = mean_miljø + se_miljø), width = 0.2) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  labs(
    title = "Climate Position by Green Party Vote Share Bins",
    x = "Green Party Vote Share (Centered)",
...
# Calculate annotation values first
max_y_value <- max(final_dataset$miljø_afhængig, na.rm = TRUE)
annotation_y_position <- max_y_value * 0.9

# Create the plot with simplified annotate calls
enhanced_rdd_plot <- ggplot(final_dataset, aes(x = centered_lagged_pervote_samlet, y = miljø_afhængig)) +
  geom_point(aes(color = factor_lagged_i_parlament), alpha = 0.5) +
  ...
  geom_smooth(data = subset(final_dataset, centered_lagged_pervote_samlet >= 0),
             ...
             color = "red", se = TRUE) +
  geom_vline(xintercept = 0, linetype = "dashed") +
  # Use geom_text instead of annotate for more reliable behavior
  geom_text(data = data.frame(
      x = c(-5, 5),
      y = c(annotation_y_position, annotation_y_position),
      label = c("Green Party Below Threshold", "Green Party Above Threshold"),
      color = c("blue", "red")
    ),
  ...
  labs(
    title = "RDD Visualization: Effect of Green Party Parliamentary Entry on Climate Position",
...
economy_facet_plot <- ggplot(final_dataset, aes(x = centered_lagged_pervote_samlet, y = miljø_afhængig)) +
  geom_point(alpha = 0.3) +
  geom_smooth(data = subset(final_dataset, centered_lagged_pervote_samlet < 0),
             method = "lm", formula = y ~ poly(x, 2, raw = TRUE), 
             color = "blue") +
...
# 6.3 Compare main climate measure with alternative measures (with limited x-axis)
# Alternative approach: Remove outliers before plotting
alt_measures_no_outliers <- alt_measures_comparison %>%
...
alt_measures_plot_no_outliers <- ggplot(alt_measures_no_outliers, aes(x = value, fill = measure)) +
  geom_density(alpha = 0.5) +
  labs(
    title = "Comparison of Alternative Climate Position Measures (Without Outliers)",
    x = "Climate Position Value",
    y = "Density",
    fill = "Measure"
  ) +
  theme_minimal()

# 6.4 Temporal evolution of climate position by party family
...
# 6.5 Climate position by country with time trends
...
# 6.7 Distribution of climate position by parliamentary status
...
# 6.8 Climate position before and after green party entry
...
# 6.9 Climate position vs. economic indicators
...
# 6.10 Ridge plot of climate position by country
...
eda_plots <- list(
  ...
  # Skip density_test_plot if it wasn't created successfully
  # density_test_plot = density_test_plot,
  ...
  conditional_means_plot = conditional_means_plot,
  enhanced_rdd_plot = enhanced_rdd_plot,
  ...
  alt_measures_plot = alt_measures_plot,
...
print(rv_dist_plot)
print(density_test_plot_substitute)  # Use the substitute
...
print(conditional_means_plot)
print(enhanced_rdd_plot)

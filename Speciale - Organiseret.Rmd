...
# Forbered økonomiske data
#gdp_per_capita_growth$year <- gdp_per_capita_growth$year
#inflation_consumer_prices$year <- inflation_consumer_prices$year
#unemployment$year <- unemployment$year

# Opret samlet økonomisk datasæt
economic_data_full <- merge(
  gdp_per_capita_growth[, c("countryname", "Year", "GDP per capita growth")],
  inflation_consumer_prices[, c("countryname", "Year", "Inflation-consumer prices")],
...
economic_data_full <- merge(
  economic_data_full,
  unemployment[, c("countryname", "Year", "Unemployment")],
  by = c("countryname", "Year"),
  all = TRUE
)

# Omdøb kolonnerne for klarhed
names(economic_data_full) <- c("countryname", "Year", "gdp_growth", "inflation", "unemployment")

# Tilføj numerisk år til final_dataset
final_dataset$year <- format(as.Date(final_dataset$edate), "%Y")
final_dataset$year_numeric <- as.numeric(final_dataset$year)

# Opret dataramme til at gemme de gennemsnitlige økonomiske variabler
avg_economic_data <- data.frame()

# For hvert land, sortér valg efter dato og beregn gennemsnit mellem valgene
for (countryname in unique(final_dataset$countryname)) {
  # Isolér data for dette land
  country_elections <- final_dataset[final_dataset$countryname == countryname, ]
  country_economics <- economic_data_full[economic_data_full$countryname == countryname, ]
  
  # Sortér valg efter dato
  country_elections <- country_elections[order(country_elections$edate), ]
  
  # For hvert valg (undtagen det første), beregn gennemsnit siden sidste valg
  if(nrow(country_elections) > 1) {  # Sikre at der er mindst 2 valg
    for (i in 2:nrow(country_elections)) {
      current_election_year <- country_elections$year_numeric[i]
      previous_election_year <- country_elections$year_numeric[i-1]
      
      # Find økonomiske data mellem disse to valg
      economic_period <- country_economics[country_economics$Year >= previous_election_year & 
                                           country_economics$Year < current_election_year, ]
      
      # Hvis der er data tilgængelige, beregn gennemsnit
      if (nrow(economic_period) > 0) {
        avg_gdp <- mean(economic_period$gdp_growth, na.rm = TRUE)
        avg_inflation <- mean(economic_period$inflation, na.rm = TRUE)
        avg_unemployment <- mean(economic_period$unemployment, na.rm = TRUE)
        
        # Tilføj til datarammen
        avg_economic_data <- rbind(avg_economic_data, 
                                 data.frame(countryname = countryname,
                                           election_year = as.character(current_election_year),
...
# Merge de gennemsnitlige data med hoveddatasættet
final_dataset_with_avg_econ <- merge(
  final_dataset,
  avg_economic_data,
  by.x = c("countryname", "year"),
  by.y = c("countryname", "election_year"),
  all.x = TRUE
)

avg_economic_data$country <- final_dataset$country[match(avg_economic_data$countryname, final_dataset$countryname)]
...
# Beregn en sammensat økonomisk indikator
# Standardiser hver variabel først (højere værdi = bedre økonomi)
final_dataset_with_avg_econ$gdp_growth_std <- scale(final_dataset_with_avg_econ$avg_gdp_growth)
final_dataset_with_avg_econ$inflation_std <- -scale(final_dataset_with_avg_econ$avg_inflation)  # Minus fordi lavere inflation er bedre
final_dataset_with_avg_econ$unemployment_std <- -scale(final_dataset_with_avg_econ$avg_unemployment)  # Minus fordi lavere arbejdsløshed er bedre

# Beregn et samlet økonomisk indeks (gennemsnit af standardiserede værdier)
final_dataset_with_avg_econ$economic_index <- rowMeans(
  final_dataset_with_avg_econ[, c("gdp_growth_std", "inflation_std", "unemployment_std")],
  na.rm = TRUE
)

# Opret binær variabel for god økonomi baseret på det sammensatte indeks
final_dataset_with_avg_econ$good_economy <- 
  ifelse(final_dataset_with_avg_econ$economic_index > 
           median(final_dataset_with_avg_econ$economic_index, na.rm = TRUE), 1, 0)
View(final_dataset_with_avg_econ)

# Simple linear model for H3
h3_model_simple <- lm(miljø_afhængig ~ centered_lagged_pervote_samlet + lagged_i_parlament,
                     data = final_dataset_with_avg_econ)
summary(h3_model_simple)

# Interaction model med økonomiske faktorer
h3_model_economic <- lm(miljø_afhængig ~ centered_lagged_pervote_samlet * lagged_i_parlament * good_economy, 
                        data = final_dataset_with_avg_econ)
summary(h3_model_economic)

# Opdel baseret på økonomisk tilstand
clean_data_good_economy <- subset(final_dataset_with_avg_econ, good_economy == 1)
clean_data_bad_economy <- subset(final_dataset_with_avg_econ, good_economy == 0)

# Clustered standard errors (by party)
if (exists("coeftest.cluster_grøn")) {
  data_used <- na.omit(final_dataset_with_avg_econ[, c("miljø_afhængig", "centered_lagged_pervote_samlet", 
                                      "lagged_i_parlament", "party", "edate", "countryname", "good_economy")])
  
  
  h3_robust <- coeftest.cluster_grøn(
    data = data_used,
    fm = h3_model_economic,
    cluster1 = data_used$party  # Pass the vector directly
  )
  print(h3_robust)
  ...
  # Multi-polynomial RDD med custom function
  if (exists("rd.base_grøn")) {
    # God økonomi sample
...
# Definer kovariater, der skal kontrolleres for
covariates_to_use <- c("countryname", "edate")

# Kør robusthedsanalysen på god økonomi data
results_df_h3_good <- run_rdd_robustness(
  ...
  polynomials = c(1, 2, 3, 4),
  covariates = covariates_to_use,
  group_label = "H3 - God økonomi"
)

# Kør robusthedsanalysen på dårlig økonomi data
...
# Opret en pæn tabel med knitr
if (requireNamespace("knitr", quietly = TRUE)) {
  # Kombinér resultaterne
  combined_results_economy <- rbind(results_df_h3_good, results_df_h3_bad)
  
  print(knitr::kable(combined_results_economy, 
                    caption = "H3: Robusthedstest opdelt efter økonomisk tilstand"))
}

# Standard RDD approach på god økonomi
...
# Standard RDD approach på dårlig økonomi
h3_rdd_bad <- rdrobust(
...
# Interaction model (simplere version)
h3_interaction <- lm(
...
# IV estimation med interaktion med økonomisk tilstand
...
# Visual confirmation - god økonomi
h3_plot_good <- plot_rdd(
  ...
  x_label = "Green Party Vote Share (centered)",
  y_label = "Climate Position",
  title = "H3: Effekt ved god økonomisk tilstand"
)
print(h3_plot_good)

# Visual confirmation - dårlig økonomi
h3_plot_bad <- plot_rdd(
  ...
  x_label = "Green Party Vote Share (centered)",
  y_label = "Climate Position",
  title = "H3: Effekt ved dårlig økonomisk tilstand"
)
print(h3_plot_bad)
...
if (requireNamespace("gridExtra", quietly = TRUE)) {
  ...
  gridExtra::grid.arrange(h3_plot_good, h3_plot_bad, ncol = 2,
                         top = "H3: Effekt opdelt efter økonomisk tilstand")
}
...
if (requireNamespace("ggplot2", quietly = TRUE) && requireNamespace("tidyr", quietly = TRUE)) {
  ...
  pred_data$predicted <- predict(h3_triple, newdata = pred_data)
  
  # Tilføj faktorer for pænere labels
  pred_data$economy_status <- factor(pred_data$good_economy, 
                                    levels = c(0, 1),
                                    labels = c("Dårlig økonomisk tilstand", "God økonomisk tilstand"))
  ...
  ggplot2::ggplot(pred_data, ggplot2::aes(x = centered_lagged_pervote_samlet, 
    ...
    ggplot2::theme_minimal() +
    ggplot2::labs(
      title = "H3: Forudsagt klimaposition baseret på økonomisk tilstand",
      x = "Grønt partis stemmeandel (centreret ved tærskel)",
      y = "Forudsagt klimaposition",
      color = "Grønt parti status",
      linetype = "Grønt parti status"
...
```{r}
# Implementation af coefficient stability plots og placebo-tests
# Dette script anvender de funktioner vi har defineret på specialets datasæt

# Først skal vi indlæse funktionerne
# Kildekode-filerne antages at være gemt i de tidligere definerede filer
source("coefficient_stability_plots.R")  # Indeholder funktionerne fra første artefakt
source("placebo_tests.R")  # Indeholder funktionerne fra andet artefakt
...
all_models_dot_whisker <- create_dotwhisker_plot(
  ...
  outcome_var = "miljø_afhængig",
  running_var = "centered_lagged_pervote_samlet",
  bw_values = c(1, 2, 3),  # Tre repræsentative båndbredder
  polynomial = 1,
  covariates = c("country", "edate"),
  title = "Sammenligning af effekter på tværs af alle modeller"
)
...
placebo_vars <- c("rile_lowe", "per410")  # Erstatter med relevante placebo variables
...
# BEMÆRK: Funktionerne er antaget at være definerede i de eksterne filer.
# Hvis du ønsker at køre denne kode direkte, skal du først kopiere definitionen af 
# alle de funktioner der er defineret i de tidligere artefakter.
```
